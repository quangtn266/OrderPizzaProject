...
public class DataGenerator {
    ...
    private static final Instant beginTime = Instant.parse(ZonedDateTime.now(ZoneOffset.UTC).format(DateTimeFormatter.ISO_INSTANT));

    private transient long rideId;

    /*
    * Create a DataGenerator for the specified reideId
    * */
    ...
    *  */
    public long driverId() {
        ...
        return 26061993 + rnd.nextInt(NUMBER_OF_DRIVERS);
    }

    /*
    * Deterministically generates and returns the taxiId for this ride
    * */
    public long taxiId() { return  driverId();}

    public float startLat() { return new Random(rideId).nextInt(10);}

    public float startLon() { return new Random(rideId).nextInt(10);}

    public String status() {
        String statuses[] = new String[]{"In-progress", "Shipped", "Delivered"};
        Random rnd = new Random(rideId);
        return statuses[rnd.nextInt(3)];
    }

    /*
    *  Deterministically generates and returns the oassengerCnt for this ride.
    * */
    public short pizzaType() { return  (short) aLong(1L, 4L);}

    /*
    *  Deterministically generates and returns the payment Type for this ride
    * */
    public String paymentType() { return (rideId%2==0) ? "CARD": "CASH";}
    ...
    * */
    public float tip() { return aLong(0L, 60L, 10F, 15F);}

    /*
    * Deterministically generates and returns the tolls for this ride
    * */
    public float tolls() { return  (rideId%10==0) ? aLong(0L, 5L):0L;}

    /*
    * Deterministically generates and returns the totalFare for this ride
    * */
    public float totalFare() { return  (float) (3.0+(1.0 + rideDurationMinutes()) + tip() + tolls());}

    /*
    * The LongRides exercise needs to have some rides with a duration > 2 hoursm but not to many.
    * */
    private long rideDurationMinutes() { return aLong(0L, 600, 20, 40);}
    ...
    // the rideId is used as the seed to guarantee deteministically results
    ...
    // the reideId is used as the seed to guarantee deteministic results
